...
const selectionCache = new Map<string, DictionarySelection[]>();

export function getCachedDictionarySelections(displayLanguage: string): DictionarySelection[] | undefined {
  const selections = selectionCache.get(displayLanguage);
  return selections ? [...selections] : undefined;
}

export async function preloadDictionarySelections(displayLanguages: readonly string[]) {
  await Promise.all(displayLanguages.map((displayLanguage) => getDictionarySelections(displayLanguage)));
}
...
export async function getDictionarySelections(displayLanguage = 'en'): Promise<DictionarySelection[]> {
  ...
  const selections = dictionaries.map((dictionary) => { const studied_word_count = counts.get(dictionary.id) ?? 0; return { ...dictionary, is_enabled: preferences.get(dictionary.id) ?? false, studied_word_count, progress_percent: dictionary.word_count ? Math.round(studied_word_count / dictionary.word_count * 100) : 0 }; });
  selectionCache.set(displayLanguage, selections);
  return selections;
}
export async function setDictionaryEnabled(dictionaryId: number, enabled: boolean) {
  if (!(await contentDictionaryExists(dictionaryId))) throw new Error(`Unknown dictionary ID: ${dictionaryId}`);
  await (await getUserDatabase()).runAsync('INSERT INTO dictionary_preferences (dictionary_id, is_enabled, updated_at) VALUES (?, ?, unixepoch()) ON CONFLICT(dictionary_id) DO UPDATE SET is_enabled=excluded.is_enabled, updated_at=excluded.updated_at', [dictionaryId, enabled ? 1 : 0]);
  for (const [language, selections] of selectionCache) {
    selectionCache.set(language, selections.map((selection) => selection.id === dictionaryId ? { ...selection, is_enabled: enabled } : selection));
  }
}
